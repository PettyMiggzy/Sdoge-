...
export function loadConfig({ env = process.env, file } = {}) {
  ...
  const cfg = {
    ...
    blockTimeMs: Number(f.blockTimeMs ?? 500),
    maintenanceIntervalMs: Number(f.maintenanceIntervalMs ?? 15000),
    txTimeoutSec: Number(f.txTimeoutSec ?? 120),
    rpcMaxConcurrent: Number(f.rpcMaxConcurrent ?? 8),
    limits: { ...f.limits },
