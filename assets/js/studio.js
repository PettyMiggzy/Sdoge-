...
async function readCollection(address) {
  ...
  const [name, symbol, owner, minted, cap, base, suffix, frozen, drop, balance, royalty, verified] = await Promise.all([
    c.name(),
    c.symbol(),
    c.owner(),
    c.totalMinted(),
  ...
  return { address, name, symbol, owner, minted, cap, base, suffix, frozen, drop, balance, royaltyBps: royalty[1], verified };
}
...
function collectionCardHtml(c) {
  ...
  const supply = c.cap > 0n ? `${fmt(c.minted)} / ${fmt(c.cap)} minted` : `${fmt(c.minted)} minted, no cap yet`;
  const dropText = d.open
    ? `Drop open: ${d.priceWei > 0n ? `${usdcText(d.priceWei)} USDC` : 'free'} each${d.maxPerWallet > 0n ? `, ${d.maxPerWallet} per wallet` : ''}`
    : 'Drop closed';
  const a = c.address;
  return `
    <div class="panel collection-card" data-collection="${a}">
      ...
      <p class="studio-note">${supply} &middot; ${dropText} &middot; royalty ${(Number(c.royaltyBps) / 100).toFixed(2)}% &middot; sales waiting: ${usdcText(c.balance)} USDC${c.frozen ? ' &middot; metadata frozen' : ''}</p>
      ${
        !mine
          ? '<p class="studio-note">You no longer own this collection.</p>'
          : `
      ...
      <label class="studio-field">Or one token per URI, one per line (up to ${MAX_URI_BATCH})<textarea class="studio-input" data-f="uris" placeholder="ipfs://.../1.json"></textarea></label>
      ...
      <div class="studio-actions">
        ...
        <button class="chip-btn" data-act="freeze" ${c.frozen ? 'disabled' : ''}>Freeze metadata</button>
      </div>
      <div class="studio-row">
        <label class="studio-field">Drop price (USDC, 0 = free)<input class="studio-input" data-f="price" value="${usdcText(d.priceWei)}" /></label>
        ...
        <label class="studio-field">Starts (optional)<input class="studio-input" data-f="start" type="datetime-local" /></label>
        <label class="studio-field">Ends (optional)<input class="studio-input" data-f="end" type="datetime-local" /></label>
      </div>
      <div class="studio-actions">
...
async function loadMyCollections() {
  ...
  try {
    const addresses = await studioRead.collectionsOf(userAddress);
    if (!addresses.length) {
      box.innerHTML = '<p class="empty-state">No collections yet. Create one above.</p>';
      return;
    }
    const cards = await Promise.all(addresses.map(readCollection));
    box.innerHTML = cards.map(collectionCardHtml).join('');
    ...
    console.error(err);
    box.innerHTML = '<p class="empty-state">Could not load your collections right now.</p>';
  }
}
...
async function collectionAction(address, act, card, btn) {
  ...
  if (act === 'mintBatch') {
    ...
    if (!ethers.isAddress(to)) return alert('Enter the address to mint to.');
    if (!uris.length || uris.length > MAX_URI_BATCH) return alert(`Enter 1-${MAX_URI_BATCH} URIs, one per line.`);
    ...
    if (!confirm('Freeze the metadata for good? The base URI and collection metadata can never change again.')) return;
    ...
    if (price === 0n && !confirm('A free drop: collectors pay nothing, and every mint uses one of your credits. Continue?')) return;
    done = await sendTx('Saving the drop', () => c.setDrop(price, perWallet, start, end, arcTx()));
  } else if (act === 'toggleDrop') {
    const d = await collectionRead(address).drop();
    if (!d.open && !(await collectionRead(address).baseURI())) return alert('Set a base URI first: drop tokens use it for their metadata.');
    done = await sendTx(d.open ? 'Closing the drop' : 'Opening the drop', () => c.setDropOpen(!d.open, arcTx()));
...
async function loadDrop() {
  ...
  try {
    ...
    const [name, symbol, owner, minted, cap, d, verified, now] = await Promise.all([
      c.name(),
      c.symbol(),
      c.owner(),
      c.totalMinted(),
    ...
    else if (ownerCredits === 0n) state = 'Paused: the creator is out of mint credits.';
...
async function loadStudio() {
  ...
  try {
    if (!(await hasCodeOnArc(STUDIO_CONTRACT_ADDRESS))) throw new Error(`no Studio contract at ${STUDIO_CONTRACT_ADDRESS} on Arc`);
    const [onChain, community] = await Promise.all([studioRead.getPackages(), studioRead.communityCollection()]);
