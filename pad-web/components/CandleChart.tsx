  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="Price chart">
      {Array.from({ length: ticks }, (_, i) => {
        ...
        return (
          <g key={i}>
            <line x1={padL} x2={W - padR} y1={m.y(p)} y2={m.y(p)} stroke="#1A2E5C" strokeDasharray="2 5" />
            <text x={W - padR + 8} y={m.y(p) + 4} fill="#A9B4D0" fontSize="11">{fmtP(p)}</text>
          </g>
        );
      })}
      {candles.map((c, i) => {
        ...
        return (
          <g key={c.t}>
            ...
            {i % labelEvery === 0 && (
              <text x={m.x(i)} y={H - 7} fill="#A9B4D0" fontSize="11" textAnchor="middle">
