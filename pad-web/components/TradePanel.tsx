...
const GAS_RESERVE_USDC = 100_000n; // $0.10 (6 decimals)
...
export function TradePanel(p: Props) {
  ...
  const inToken = isBuy ? CONFIG.usdc : p.token;
  const outToken = isBuy ? p.token : CONFIG.usdc;
  ...
  // Both are real ERC-20 reads — never the wallet's native balance. Arc's
  // native currency mirrors the USDC predeploy 1:1, but the contracts only
  // ever move USDC via ERC-20 transferFrom, so the balance shown must come
  // from the same asset the swap actually spends.
  const usdcBal = useReadContract({ address: CONFIG.usdc, abi: erc20Abi, functionName: 'balanceOf', args: [address!], chainId: arc.id, query: { enabled: !!address, refetchInterval: 15_000 } });
  const tokenBal = useReadContract({ address: p.token, abi: erc20Abi, functionName: 'balanceOf', args: [address!], chainId: arc.id, query: { enabled: !!address, refetchInterval: 15_000 } });
  const inBal = isBuy ? usdcBal.data : tokenBal.data;
  ...
  const spendable = inBal === undefined ? undefined : isBuy ? (inBal > GAS_RESERVE_USDC ? inBal - GAS_RESERVE_USDC : 0n) : inBal;
  ...
  async function submit() {
    ...
    try {
      ...
      await waitOk(h, 'Swap');
      setAmount(''); usdcBal.refetch(); tokenBal.refetch();
    } catch (e: unknown) {
  ...
  const balLabel = inBal === undefined ? '—' : isBuy
    ? `${Number(formatUnits(inBal, CONFIG.quoteDecimals)).toFixed(2)} USDC`
  ...
  const outSymbol = isBuy ? p.symbol : 'USDC';
  ...
  const buttonLabel = busy
    ?? (needsGasRoom ? 'Leave $0.10 for gas'
  ...
  return (
    <div className="panel p-5">
      <div className="grid grid-cols-2 rounded-xl2 bg-panel2 p-1">
        <button className={clsx('rounded-xl py-2.5 text-sm font-bold', isBuy ? 'bg-sdoge-grad text-white shadow-btn' : 'text-muted')} onClick={() => setSide('buy')}>Buy</button>
        <button className={clsx('rounded-xl py-2.5 text-sm font-bold', !isBuy ? 'bg-sdoge-grad text-white shadow-btn' : 'text-muted')} onClick={() => setSide('sell')}>Sell</button>
      </div>

      <div className="mt-5 flex items-center justify-between text-sm">
        <span className="font-semibold">{isBuy ? 'Amount in USDC' : `Amount in ${p.symbol}`}</span>
      ...
      {noLiquidity && amountIn > 0n && (
        <div className="mt-3 rounded-xl border border-line2 bg-panel2 p-2 text-xs text-muted">
          {isBuy ? 'Nothing left in the pool to buy at the moment.' : `No USDC in the pool yet. Once someone buys ${p.symbol}, selling opens up.`}
        </div>
      )}
