...
const DAY = 86_400;
const SLOTS = 4; // launch cards after the pinned $SDOGE card
  ...
  return (
    <div className="panel flex flex-col p-4 transition hover:border-brand-hi/60">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            <img src="/brand/arc-mark.png" alt="Arc" className="h-4 w-auto" />
      ...
      <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-line" title={`Progress to a ${fmtUsd(CONFIG.milestoneUsd, { compact: true })} market cap`}>
        <div className="h-full rounded-full bg-sdoge-grad" style={{ width: `${pct}%` }} />
      </div>
...
type DexPair = { chainId: string; url: string; marketCap?: number; fdv?: number; priceChange?: { h24?: number }; liquidity?: { usd?: number } };

/** $SDOGE trades outside this pad, so its numbers come live from DexScreener. */
function SdogeCard() {
  const q = useQuery({
    queryKey: ['dexscreener', CONFIG.sdogeToken],
    refetchInterval: 60_000,
    queryFn: async () => {
      const r = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${CONFIG.sdogeToken}`);
      if (!r.ok) throw new Error(`DexScreener ${r.status}`);
      const pairs = ((await r.json()).pairs ?? []) as DexPair[];
      return pairs.filter((p) => p.chainId === 'arc').sort((a, b) => (b.liquidity?.usd ?? 0) - (a.liquidity?.usd ?? 0))[0] ?? null;
    },
  });
  const pair = q.data;
  const mcap = pair ? Number(pair.marketCap ?? pair.fdv) || undefined : undefined;
  const href = pair?.url ?? `https://dexscreener.com/arc/${CONFIG.sdogeToken}`;
  return (
    <Card
      // eslint-disable-next-line @next/next/no-img-element
      avatar={<img src="/brand/logo.jpg" alt="" className="h-full w-full object-cover" />}
      name="SDOGE" featured pill={{ text: 'Live', tone: 'live' }} mcap={mcap} change={pair?.priceChange?.h24}
      action={<a href={href} target="_blank" rel="noreferrer" className="btn-brand w-full py-2.5">Buy Now</a>}
    />
  );
}
