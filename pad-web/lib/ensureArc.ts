...
// wagmi's own record of the connection can say Arc while the wallet itself
...
/**
 * Resolves once the wallet is on Arc: asks it to switch (adding Arc first if
...
export function useEnsureArc() {
  ...
  return useCallback(async () => {
    ...
    if ((await connector.getChainId()) === arc.id) return;
    try {
      // A phone wallet may never answer; the check below decides either way.
      await Promise.race([
        switchChainAsync({ connector, chainId: arc.id }),
      ...
      if (e instanceof UserRejectedRequestError || /rejected|denied/i.test(String((e as Error)?.message))) {
        throw new Error(`Your wallet said no to switching to ${arc.name}. Switch it to ${arc.name} and try again.`);
      }
    }
    if ((await connector.getChainId()) !== arc.id) {
      throw new Error(`Your wallet is still on another network. Open your wallet, switch its network to ${arc.name}, then come back and try again.`);
    }
