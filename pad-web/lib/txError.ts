...
// Every custom error the SDOGE Pad contracts, and the Uniswap v4 / Permit2 /
...
export const knownErrorsAbi = parseAbi([
  // SdogePadPortal, SdogePadHook, SdogePadLocker, SdogePadRevenueSplitter
  'error TaxTooHigh()',
...
const MESSAGES: Record<string, string> = {
  ...
  NoLiquidityToFill: 'No USDC in this pool yet: someone has to buy before anyone can sell.',
...
const GAS_HELP = 'This wallet is short on USDC for gas. Arc charges gas in USDC, so leave roughly $0.10 on top of the amount you trade.';
 ...
 * Arc pays gas in USDC, the same balance you trade with, and wallets reserve
...
export async function ensureGasFunds(pc: PublicClient, account: Address, gas: bigint) {
  ...
  if (balance < need) {
    const usdc = (v: bigint) => Number(formatUnits(v, 18)).toFixed(4);
    throw new Error(`Short on gas: this needs about ${usdc(need)} USDC and the wallet holds ${usdc(balance)} USDC. USDC doubles as Arc's gas coin.`);
  }
}
