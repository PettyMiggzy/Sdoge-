        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {(['chart', 'trades', 'holders', 'info'] as Tab[]).map((t) => (
              <button key={t} className={clsx('rounded-xl px-4 py-2 text-sm font-bold capitalize', tab === t ? 'bg-sdoge-grad text-white shadow-btn' : 'bg-panel text-muted hover:text-text')} onClick={() => setTab(t)}>
                {t}
              </button>
            ))}
            {tab === 'chart' && (
