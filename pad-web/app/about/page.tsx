    ...
    <div className="max-w-3xl space-y-8">
      ...
      <section className="panel space-y-3 p-6">
        ...
        <p className="text-text/85">
          {CONFIG.brand} is Stable Doge&apos;s launchpad on {CONFIG.chainName}, open to anyone. A new token
          gets its own Uniswap v4 pool, paired with USDC, and can be traded in the same block it was created.
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <Fact icon={Coins} title="Same start for every doge" body="Each launch mints exactly 1,000,000,000 tokens and all of them go into the pool. No team bag, no presale. The creator buys from the pool like everyone else." />
        ...
        <Fact icon={Percent} title="A split set in code" body="A launch's revenue (its tax plus the LP fees it earns in USDC) goes 90% to the creator and 10% to the platform. That ratio is a constant in the contract, not a setting." />
        <Fact icon={Eye} title="Real numbers or a dash" body="Numbers here are read from the chain, from the indexer, or (for $SDOGE itself) from DexScreener. When there is no real figure to show, you get a “—”, not a guess." />
      </section>

      <section className="panel space-y-2 p-6 text-sm text-muted">
        <h2 className="font-bold text-text">Before you buy</h2>
        <p>
          Much fun, but please be careful: this is experimental software that handles real USDC on {CONFIG.chainName}.
