  ...
  async function submit() {
    ...
    try {
      const call = {
        ...
        // CreateLaunchParams has no quoteAsset: SdogePadPortal fixes its quote
        // asset (USDC) at construction. portalAbi in lib/abi.ts matches the
        // struct exactly.
        args: [{
          name: name.trim(),
          symbol,
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 font-semibold uppercase tracking-wide text-muted">One transaction. Its own USDC pool. Wow.</p>
      </div>
          ...
          <div className="panel space-y-5 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-[1fr_1fr_auto]">
              <div>
                ...
                <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Much Wow Coin" maxLength={32} />
              </div>
              <div>
                <label className="label">Ticker</label>
                <input className="input" value={symbol} onChange={(e) => setSymbol(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ''))} placeholder="WOW" maxLength={10} />
              </div>
              <div>
        ...
        <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          <div className="panel p-5">
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <div>
                <div className="font-bold">{name || 'Nameless doge'}</div>
          ...
          <div className="panel p-5">
            ...
            <ul className="mt-3 space-y-2 text-sm text-text/85">
              {['No team tokens, no presale', 'Rules you can read on-chain', 'Each pool trades against USDC'].map((t) => (
      ...
      <div className="panel grid grid-cols-2 gap-4 p-6 sm:grid-cols-4">
        {[['1', 'Name it', 'Ticker, picture, a short story'], ['2', 'Launch it', 'Its USDC pool opens at once'], ['3', 'Share it', 'Rally the doges'], ['4', 'Build it', 'Keep showing up for holders']].map(([n, t, d]) => (
          <div key={n} className="flex items-center gap-3">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-sdoge-grad font-display text-sm font-bold text-white">{n}</div>
