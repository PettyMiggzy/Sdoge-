...
/// @notice Regression tests for the 2026-09-24 adversarial audit. Every tax
/// assertion is exact, and each swap shape runs in both pool orientations.
contract AuditFixesTest is Test, Deployers {
    ...
    function _hook() internal returns (SdogePadHook h) {
        bytes memory args = abi.encode(address(manager), address(this));
        (address predicted, bytes32 salt) = HookMiner.find(address(this), FLAGS, type(SdogePadHook).creationCode, args);
        h = new SdogePadHook{salt: salt}(address(manager), address(this));
    ...
    function _launch(address quoteAt) internal returns (Env memory e) {
        ...
        e.hook = _hook();
        e.treasury = new SdogePadTreasury(makeAddr("treasuryOwner"));
        e.portal = new SdogePadPortal(address(manager), address(e.hook), address(e.treasury), quoteAt, true);
        ...
        e.splitter = SdogePadLocker(e.locker).splitter();
        ...
        SdogePadLaunchToken(e.token).approve(address(swapRouter), type(uint256).max);
        vm.stopPrank();
    }
    ...
    // ------------------------------------------------------------------
    // hook-3: every swap shape, both orientations, exact tax
    // ------------------------------------------------------------------
    ...
    function _allShapes(address quoteAt) internal {
        ...
        uint256 bal = SdogePadLaunchToken(e.token).balanceOf(trader);
        p0 = _pending(e);
        ...
        assertEq(SdogePadRevenueSplitter(e.splitter).creditedToPlatform(quoteAt), platform, "main pad platform share");
        assertEq(SdogePadRevenueSplitter(e.splitter).creditedToCreator(quoteAt), total - platform, "creator share");
        assertEq(SdogePadRevenueSplitter(e.splitter).creditedToCreator(e.token), 0, "never credited in the launch token");
        assertEq(SdogePadLaunchToken(e.token).balanceOf(e.splitter), 0, "splitter never holds the launch token");
    ...
    function _opensAtEdgeAndBlocksFreePush(address quoteAt) internal {
        ...
        int24 edge = e.tokenIsToken0 ? SdogePadLocker(e.locker).tickLower() : SdogePadLocker(e.locker).tickUpper();
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                abi.encodeWithSelector(SdogePadHook.NoLiquidityToFill.selector),
    ...
    function test_ThirdPartyRangeOrderIsRejected() public {
        ...
        vm.startPrank(attacker);
        SdogePadLaunchToken(e.token).approve(address(modifyLiquidityRouter), type(uint256).max);
        ...
        for (uint256 i; i < 2; i++) {
            vm.expectRevert(
                abi.encodeWithSelector(
                    ...
                    abi.encodeWithSelector(SdogePadHook.LiquidityLocked.selector),
                    abi.encodeWithSelector(Hooks.HookCallFailed.selector)
                )
            );
    ...
    function test_PartialFillOfTaxedExactInBuyReverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                address(e.hook),
                IHooks.afterSwap.selector,
                abi.encodeWithSelector(SdogePadHook.PartialFillUnsupported.selector),
                abi.encodeWithSelector(Hooks.HookCallFailed.selector)
            )
        );
        swapRouter.swap(
    ...
    function test_BootstrapFactoryRejectsNonFactoriesAndMiswiredFactories() public {
        SdogePadHook h = _hook();
        vm.expectRevert(SdogePadHook.InvalidFactory.selector);
        h.bootstrapFactory(makeAddr("eoa"));

        SdogePadHook other = _hook();
        SdogePadFactory wrong =
            new SdogePadFactory(address(manager), address(other), makeAddr("treasury"), address(1), 100e6, address(this));
        vm.expectRevert(SdogePadHook.InvalidFactory.selector);
        h.bootstrapFactory(address(wrong));

        SdogePadFactory right =
            new SdogePadFactory(address(manager), address(h), makeAddr("treasury"), address(1), 100e6, address(this));
    ...
    function test_RenounceClosesTheFactorySlotForGood() public {
        SdogePadHook h = _hook();
        vm.prank(attacker);
        vm.expectRevert(SdogePadHook.NotBootstrapper.selector);
        ...
        SdogePadFactory f =
            new SdogePadFactory(address(manager), address(h), makeAddr("treasury"), address(1), 100e6, address(this));
        vm.expectRevert(SdogePadHook.AlreadyBootstrapped.selector);
        h.bootstrapFactory(address(f));
    }

    function test_FactoryConstructorRejectsBadWiring() public {
        SdogePadHook h = _hook();
        vm.expectRevert(SdogePadFactory.HookMismatch.selector);
        new SdogePadFactory(makeAddr("otherManager"), address(h), makeAddr("t"), address(1), 100e6, address(this));
        vm.expectRevert(SdogePadFactory.FeeTooHigh.selector);
        new SdogePadFactory(address(manager), address(h), makeAddr("t"), address(1), 100e18, address(this));
        vm.expectRevert(SdogePadFactory.ZeroAddress.selector);
        new SdogePadFactory(address(manager), address(h), address(0), address(1), 100e6, address(this));
    }
    ...
    function test_PortalCannotRegisterAPoolItDidNotInitialize() public {
        ...
        vm.expectRevert(SdogePadHook.NotInitializer.selector);
        e.hook.registerPool(squat, e.splitter, e.locker, LOW_QUOTE, false, 100, 100);
    }

    // ------------------------------------------------------------------
    // Design rule: no trading restrictions.
    ...
    function _scannerProfile(address quoteAt) internal {
        ...
        assertEq(SdogePadLaunchToken(e.token).balanceOf(trader), 0);

        // Wallet-to-wallet transfers are plain ERC-20: no tax, no restriction.
        (, int256 again) = _swap(e, true, -int256(10e6));
        address friend = makeAddr("friend");
        vm.prank(trader);
        SdogePadLaunchToken(e.token).transfer(friend, uint256(again));
        assertEq(SdogePadLaunchToken(e.token).balanceOf(friend), uint256(again), "transfers are never taxed");
    }
    ...
    function test_SweepSurplusCreditsStrayQuoteAndSelfClaimIsRejected() public {
        ...
        SdogePadRevenueSplitter(e.splitter).sweepSurplus(LOW_QUOTE);
        assertEq(SdogePadRevenueSplitter(e.splitter).creditedToPlatform(LOW_QUOTE), 100e6);
        assertEq(SdogePadRevenueSplitter(e.splitter).creditedToCreator(LOW_QUOTE), 900e6);

        vm.expectRevert(SdogePadRevenueSplitter.NothingToClaim.selector);
        SdogePadRevenueSplitter(e.splitter).sweepSurplus(LOW_QUOTE);

        vm.prank(creator);
        vm.expectRevert(SdogePadRevenueSplitter.InvalidRecipient.selector);
        SdogePadRevenueSplitter(e.splitter).claim(e.splitter, LOW_QUOTE);
    }
}
