...
/// importing SdogePadFactory, which imports this contract.
interface IFactoryWiring {
    function hook() external view returns (address);
    function poolManager() external view returns (address);
}

/// @notice ONE shared hook contract, used by every SDOGE Pad launch's pool
/// — including every white-label pad deployed through SdogePadFactory, not
/// just the original SDOGE Pad. Every launch is a real Uniswap v4 pool from
/// block one (no bonding curve, no graduation — see SdogePadPortal), and this
...
contract SdogePadHook is IHooks, IUnlockCallback {
    ...
    struct PoolConfig {
        ...
        // positions for existing readers (the site, the indexer).
        address locker; // the only address allowed to add liquidity to this pool
    }

    uint16 public constant MAX_TAX_BPS = 1_000; // 10% per side — mirrors SdogePadPortal.MAX_TAX_BPS (defense in depth)
    ...
    //   - `bootstrapMainPortal`: wires up SDOGE Pad's own, original Portal.
    //   - `bootstrapFactory`: wires up the ONE trusted SdogePadFactory,
    ...
    // SdogePadHook{salt: salt}(...)` so its address lands on the required flag
    ...
    // (0x4e59b448...) rather than directly from the broadcaster's EOA.
    ...
    /// @notice Wires up SDOGE Pad's own original Portal. Callable exactly
    ...
    /// @notice Wires up the one trusted SdogePadFactory. Callable exactly
    ...
    function flush(PoolKey calldata key) external {
        ...
        ISdogePadSplitter(cfg.splitter).depositRevenue(cfg.quoteAsset, amount);
