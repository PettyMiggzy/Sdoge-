...
import {SdogePadRevenueSplitter} from "./SdogePadRevenueSplitter.sol";
...
/// supply — no bonding curve, no graduation event: real liquidity from
/// the first block. The full 1B supply is deposited as a single
...
contract SdogePadPortal {
    using PoolIdLibrary for PoolKey;
    using SafeERC20 for IERC20;

    address public immutable poolManager;
    address public immutable hook; // the one shared SdogePadHook instance (see SdogePadHook.sol)
    address public immutable treasury; // shared across every launch — see SdogePadTreasury.sol
    // Fixed per portal, not chosen by each caller (audit finding M-1: a
    ...
    // True only for SDOGE Pad's own original Portal, deployed directly (not
    // through SdogePadFactory). Passed down to each launch's
    // SdogePadRevenueSplitter, which uses it to decide the platform/creator
    ...
    struct CreateLaunchParams {
        ...
        // very first trade. Capped at 10% per side (MAX_TAX_BPS); either
        // can be 0.
        uint16 buyTaxBps;
        uint16 sellTaxBps;
    }
    ...
    function createLaunch(CreateLaunchParams calldata p) external returns (address token, address locker) {
        ...
        SdogePadLaunchToken tokenContract = new SdogePadLaunchToken(p.name, p.symbol, TOTAL_SUPPLY, address(this));
        token = address(tokenContract);

        SdogePadRevenueSplitter splitter = new SdogePadRevenueSplitter(msg.sender, treasury, address(this), isMainPad);
    ...
    function _seedLaunchPool(
        ...
        // The standard amount0/amount1 -> sqrtPriceX96 conversion:
        ...
        SdogePadRevenueSplitter(splitter).authorizeSource(hook);
        SdogePadRevenueSplitter(splitter).authorizeSource(r.locker);
        SdogePadRevenueSplitter(splitter).lockSources();
