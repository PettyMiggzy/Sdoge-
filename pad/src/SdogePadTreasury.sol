...
/// SdogePadFactory white-label one): every launch's platform cut lands here
...
/// (buying back $SDOGE, or anything else) is a deliberate, manual, human
/// decision made by whoever holds `owner`, on whatever cadence they want.
///
/// Deliberately not a "buyback-and-burn" contract: auto-swapping and
/// burning on every deposit would add real complexity and risk (a live
/// price read for slippage protection, a dependency on Arc's
/// UniversalRouter/Permit2/StateView addresses being exactly right, an
/// unsupervised swap running on every single trade). If holders want to
/// burn their own tokens, they can do that themselves.
contract SdogePadTreasury {
