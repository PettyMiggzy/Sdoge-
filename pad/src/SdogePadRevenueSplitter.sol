...
/// SdogePadLocker — between the platform treasury and the creator. On the
/// main pad: 10% to the platform, 90% to the creator. On a
/// SdogePadFactory white-label pad: 15/85 instead — the extra 5% covers
...
/// generated the revenue in the first place (see SdogePadHook's audit fix,
/// H-2).
contract SdogePadRevenueSplitter is ISdogePadSplitter {
    ...
    address public immutable treasury; // SDOGE Pad's shared plain USDC treasury — see SdogePadTreasury.sol
    address public immutable portal; // the only address allowed to authorize sources, until locked
    bool public immutable isMainPad; // see SdogePadPortal.isMainPad — decides the split ratio above
    ...
    /// SdogePadLocker right after deploying them, in the same transaction the
    ...
    /// (swap tax) and this launch's own SdogePadLocker (harvested LP fees), in
