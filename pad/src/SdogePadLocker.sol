...
/// fees, split the same way as everything else on SDOGE Pad.
contract SdogePadLocker is IUnlockCallback {
    ...
    /// SdogePadPortal._seedLaunchPool), owned by this locker from the moment
    ...
    function _payout(Currency currency, uint256 amount, bool isLaunchToken) internal {
        ...
        if (isLaunchToken) {
            ...
            ISdogePadSplitter(splitter).depositRevenue(asset, amount);
        }
    }
