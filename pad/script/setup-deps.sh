...
# In this repo the same pins are registered as submodules, so
# `git submodule update --init --recursive pad/lib` from the repo root does
# the same job; this script is for a plain copy of pad/ without git
# submodules, and it also refuses a checkout that has drifted off a pin.
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p lib
